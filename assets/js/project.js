    ...
    document.querySelectorAll('.project-card').forEach(card => {
        card.addEventListener('click', async () => {
          ...
          `;
            ...
            // Links bar at bottom (separate modal-media-box)
            if (card.dataset.linksSrc) {
                const links = await loadJSON(card.dataset.linksSrc);
                if (Array.isArray(links) && links.length > 0) {
                    const linksBox = document.createElement('div');
                    linksBox.className = 'modal-media-box';
                    const inner = document.createElement('div');
                    inner.className = 'modal-links bottom-links';
                    links.forEach(link => inner.appendChild(buildLinkBadge(link)));
                    linksBox.appendChild(inner);
                    modalMedia.appendChild(linksBox);
                }
            }
        });
    });
